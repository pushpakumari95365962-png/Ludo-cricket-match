...
<head>
    ...
    <title>Ludo Cricket MTSC Edition</title>
    <style>
        ...
        .mtsc-banner { background: var(--mtsc); color: white; padding: 5px; border-radius: 8px; font-size: 12px; font-weight: bold; margin-bottom: 10px; cursor: pointer; animation: blink 1.5s infinite; }
        ...
        .active { border-color: #4af2ff; background-color: #1c2128; transform: scale(1.05); }
...
<div class="game-box">
    <div class="mtsc-banner" onclick="showMTSC()">🏆 MTSC TOURNAMENT IS LIVE! (Click for Info)</div>
    
    <h1 style="color: var(--accent); margin: 0 0 10px 0;">Ludo-Cricket Pro 🏏</h1>
    ...
    <div class="players-container">
        <div id="p1-card" class="player-card active">
            ...
            <small>Wkts: <span id="p1-wickets">0</span>/2</small><br>
            <small>Balls: <span id="p1-balls">12</span></small>
        </div>
        <div id="p2-card" class="player-card">
            ...
            <small>Wkts: <span id="p2-wickets">0</span>/2</small><br>
            <small>Balls: <span id="p2-balls">12</span></small>
        </div>
    </div>
...
<script>
    ...
    function showMTSC() {
        alert("--- MTSC Tournament Info ---\n\nMonthly Talent Search Championship\nPrize: 50,000 Coins + 100 Diamonds\nRule: Sabse zyada Monthly Wins laane wala player 'State Hero' banega!");
    }
    ...
    function playTurn() {
        ...
        if (result === "OUT") {
            if (lifeline[currentPlayer]) {
                ...
                status.innerHTML = `🛡️ OUT! Life-line ne bacha liya!`;
            } else {
            ...
            else status.innerHTML = `🚀 SIXER! Extra Turn mila!`;
        }
        updateUI();
    }
    ...
    function sendEmoji(e) {
        if (wallet.diamonds > 0) {
            wallet.diamonds--;
            document.getElementById("status-msg").innerHTML = `Emoji ${e} Bheja Gaya!`;
            updateUI();
        } else {
            alert("Diamonds khatam!");
        }
    }
    ...
    function checkGameOver() {
        let msg = "";
        let over = false;
        if (scores[0] >= target) { msg = "🏆 P1 WIN! +100 Coins"; wallet.coins += 100; over = true; }
        else if (scores[1] >= target) { msg = "🏆 P2 WIN! +100 Coins"; wallet.coins += 100; over = true; }
        else if (wickets[0] >= 2 && wickets[1] >= 2) { msg = "GAME OVER: All Out!"; over = true; }
        else if (balls[0] <= 0 && balls[1] <= 0) { msg = "GAME OVER: Balls Finished!"; over = true; }

        if (over) {
            document.getElementById("status-msg").innerHTML = msg;
            document.getElementById("status-msg").style.background = "#2e7d32";
            return true;
        }
        return false;
    }
